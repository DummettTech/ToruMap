...
export function createMapAreaFeature({
  ...
  // Store both raw HTML and plain text name
  const nameHtml = name;
  const nameText =
    typeof name === "string"
      ? name.replace(/<[^>]+>/g, "") // strip HTML tags for map label
      : name;
  ...
  feature.set("name", nameText); // plain text for map label
  feature.set("nameHtml", nameHtml); // raw HTML for popups
  ...
  const defaultStyleFn = function (feature, resolution) {
    const zoom = Math.log2(2 / resolution) + 2;
    const fontSize = Math.max(12, Math.min(28, Math.round(zoom * 2 + 10))); // 12–28px
    ...
    return new Style({
      ...
      text: new Text({
        text: feature.get("name") || nameText,
...
export function createAreaVectorLayer(source) {
  return new VectorLayer({ source, declutter: false });
}
